...
任务 06 扩展同一 `OperationSpec → Pipeline → Document` 核心。Document 是从原始资产和不可变 ops 求值得到的完整编辑状态；渲染 Raster 是输出，不是另一份编辑状态。没有模型、自动选区、智能抠图或图像生成调用。分组、文字、剪贴蒙版和调整层现已由任务 07 实现，扩展语义见 [分组与文字契约](groups-text-adjustments.md)。
...
任务 06 引入的检查点格式现由任务 07 升为 `PICDOC03`（旧 `PICDOC02` 按 ops 重建）：magic、64 字节 key、u64 JSON 元数据长度、完整 DocumentInfo／预览映射元数据、各 Raster 的 u32 宽高和 RGBA 四个 f32 little-endian 原始位，最后 SHA-256。多层检查点保存渲染结果及**每个独立图层、每个蒙版、变换、名称、显隐、混合、选区和已使用 ID**。恢复检查完整性、维度、目标、映射与预算；即使 key／校验和有效，多层前缀也拒绝扁平 Raster 检查点。丢失或损坏后从较早完整前缀或原资产重建。

旧单画布检查点及无额外映射的画布预览仍使用 `PICFLT01`。当前渲染缓存身份为 layers-v2，并包含组、clip、调整和文字依赖版本；旧缓存失效只影响速度，不影响工程可读性。内存预算计入全部独立 Raster 与序列化元数据估计，磁盘预算按实际文件字节，读写 scratch 按同时存在的快照和像素准入；沿用缓存锁及淘汰边界，不删除资产。多层状态与渲染缓冲也受 max_buffer_bytes 准入；这仍不是严格进程 RSS 保证。
