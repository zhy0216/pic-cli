...
检查点必须保留原始 f32 位模式、通道顺序、宽高、色彩语义及 alpha 模式。任务 05 已以版本化 little-endian IEEE-754 快照实现单画布检查点；格式、身份、预算和回退见 [重放与预览契约](replay-preview.md)。不能用 RGBA8 PNG、JPEG、缩略图替换工作像素，也不能仅保存扁平预览来恢复多图层状态；PICDOC02 已保存完整可编辑层和蒙版。
