...
记录日期：2026-09-19 UTC。对应任务 01、pic-cli 0.1.0 及本仓库 `Cargo.lock`。这是任务 01 时点的启动与 codec 历史基线，不代表当前全部能力或测试数量。照片级编辑、图层与工程的后续测量见 [当前性能报告](performance.md)；本页原始数字和环境保留供追溯。
...
任务 01 当时尚未测量冷文件缓存、真实照片/1080p/4K/高像素输入的端到端耗时和峰值内存、几何/调色/滤镜、图层、工程冷重放/检查点/预览，以及长管线的性能。任务 11 已补齐固定合成素材的 1080p/4K、调色/模糊/图层、20 步工程重放/检查点/预览/旧步骤修改；仍不是冷磁盘或真实照片集基准。资源预算目前是准入估算，不能等同全进程 RSS 上限。底层物理磁盘、宿主争用和其他进程影响没有隔离。

任务 02 已验证 EXIF 自动方向和明确的编码参数；JPEG 色彩管理、元数据保留、其他格式/位深、Photoshop 参数/输出兼容和跨平台发布仍受 [当前矩阵](capability-matrix.md) 的限制。未承诺比 Photoshop、libvips 或其他 CLI 更快，也未把这些小图数字作为产品 SLO。智能编辑仅为未来 roadmap，不属于本轮选型、测量或交付要求。
