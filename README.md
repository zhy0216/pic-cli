...
当前 **0.1.0** 已实现 PNG/JPEG 信息查询、EXIF 方向归一化、编解码参数及裁剪、缩放、旋转、翻转、画布大小的单命令与有序 JSON 管线。调色、滤镜、图层、文字、工程持久化与预览仍未实现。请以 `capabilities` 为机器可读的实际支持列表。
...
crates/pic-core/src/
  ...
  operation.rs, operation/    操作规格、参数校验、几何采样与统一执行接口
